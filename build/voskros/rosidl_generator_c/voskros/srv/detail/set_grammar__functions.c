// generated from rosidl_generator_c/resource/idl__functions.c.em
// with input from voskros:srv/SetGrammar.idl
// generated code does not contain a copyright notice
#include "voskros/srv/detail/set_grammar__functions.h"

#include <assert.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>

#include "rcutils/allocator.h"

// Include directives for member types
// Member `list`
#include "rosidl_runtime_c/string_functions.h"

bool
voskros__srv__SetGrammar_Request__init(voskros__srv__SetGrammar_Request * msg)
{
  if (!msg) {
    return false;
  }
  // list
  if (!rosidl_runtime_c__String__Sequence__init(&msg->list, 0)) {
    voskros__srv__SetGrammar_Request__fini(msg);
    return false;
  }
  return true;
}

void
voskros__srv__SetGrammar_Request__fini(voskros__srv__SetGrammar_Request * msg)
{
  if (!msg) {
    return;
  }
  // list
  rosidl_runtime_c__String__Sequence__fini(&msg->list);
}

bool
voskros__srv__SetGrammar_Request__are_equal(const voskros__srv__SetGrammar_Request * lhs, const voskros__srv__SetGrammar_Request * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // list
  if (!rosidl_runtime_c__String__Sequence__are_equal(
      &(lhs->list), &(rhs->list)))
  {
    return false;
  }
  return true;
}

bool
voskros__srv__SetGrammar_Request__copy(
  const voskros__srv__SetGrammar_Request * input,
  voskros__srv__SetGrammar_Request * output)
{
  if (!input || !output) {
    return false;
  }
  // list
  if (!rosidl_runtime_c__String__Sequence__copy(
      &(input->list), &(output->list)))
  {
    return false;
  }
  return true;
}

voskros__srv__SetGrammar_Request *
voskros__srv__SetGrammar_Request__create(void)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  voskros__srv__SetGrammar_Request * msg = (voskros__srv__SetGrammar_Request *)allocator.allocate(sizeof(voskros__srv__SetGrammar_Request), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(voskros__srv__SetGrammar_Request));
  bool success = voskros__srv__SetGrammar_Request__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
voskros__srv__SetGrammar_Request__destroy(voskros__srv__SetGrammar_Request * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    voskros__srv__SetGrammar_Request__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
voskros__srv__SetGrammar_Request__Sequence__init(voskros__srv__SetGrammar_Request__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  voskros__srv__SetGrammar_Request * data = NULL;

  if (size) {
    data = (voskros__srv__SetGrammar_Request *)allocator.zero_allocate(size, sizeof(voskros__srv__SetGrammar_Request), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = voskros__srv__SetGrammar_Request__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        voskros__srv__SetGrammar_Request__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
voskros__srv__SetGrammar_Request__Sequence__fini(voskros__srv__SetGrammar_Request__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      voskros__srv__SetGrammar_Request__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

voskros__srv__SetGrammar_Request__Sequence *
voskros__srv__SetGrammar_Request__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  voskros__srv__SetGrammar_Request__Sequence * array = (voskros__srv__SetGrammar_Request__Sequence *)allocator.allocate(sizeof(voskros__srv__SetGrammar_Request__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = voskros__srv__SetGrammar_Request__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
voskros__srv__SetGrammar_Request__Sequence__destroy(voskros__srv__SetGrammar_Request__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    voskros__srv__SetGrammar_Request__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
voskros__srv__SetGrammar_Request__Sequence__are_equal(const voskros__srv__SetGrammar_Request__Sequence * lhs, const voskros__srv__SetGrammar_Request__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!voskros__srv__SetGrammar_Request__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
voskros__srv__SetGrammar_Request__Sequence__copy(
  const voskros__srv__SetGrammar_Request__Sequence * input,
  voskros__srv__SetGrammar_Request__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(voskros__srv__SetGrammar_Request);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    voskros__srv__SetGrammar_Request * data =
      (voskros__srv__SetGrammar_Request *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!voskros__srv__SetGrammar_Request__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          voskros__srv__SetGrammar_Request__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!voskros__srv__SetGrammar_Request__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `error`
// already included above
// #include "rosidl_runtime_c/string_functions.h"

bool
voskros__srv__SetGrammar_Response__init(voskros__srv__SetGrammar_Response * msg)
{
  if (!msg) {
    return false;
  }
  // error
  if (!rosidl_runtime_c__String__init(&msg->error)) {
    voskros__srv__SetGrammar_Response__fini(msg);
    return false;
  }
  return true;
}

void
voskros__srv__SetGrammar_Response__fini(voskros__srv__SetGrammar_Response * msg)
{
  if (!msg) {
    return;
  }
  // error
  rosidl_runtime_c__String__fini(&msg->error);
}

bool
voskros__srv__SetGrammar_Response__are_equal(const voskros__srv__SetGrammar_Response * lhs, const voskros__srv__SetGrammar_Response * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // error
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->error), &(rhs->error)))
  {
    return false;
  }
  return true;
}

bool
voskros__srv__SetGrammar_Response__copy(
  const voskros__srv__SetGrammar_Response * input,
  voskros__srv__SetGrammar_Response * output)
{
  if (!input || !output) {
    return false;
  }
  // error
  if (!rosidl_runtime_c__String__copy(
      &(input->error), &(output->error)))
  {
    return false;
  }
  return true;
}

voskros__srv__SetGrammar_Response *
voskros__srv__SetGrammar_Response__create(void)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  voskros__srv__SetGrammar_Response * msg = (voskros__srv__SetGrammar_Response *)allocator.allocate(sizeof(voskros__srv__SetGrammar_Response), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(voskros__srv__SetGrammar_Response));
  bool success = voskros__srv__SetGrammar_Response__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
voskros__srv__SetGrammar_Response__destroy(voskros__srv__SetGrammar_Response * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    voskros__srv__SetGrammar_Response__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
voskros__srv__SetGrammar_Response__Sequence__init(voskros__srv__SetGrammar_Response__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  voskros__srv__SetGrammar_Response * data = NULL;

  if (size) {
    data = (voskros__srv__SetGrammar_Response *)allocator.zero_allocate(size, sizeof(voskros__srv__SetGrammar_Response), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = voskros__srv__SetGrammar_Response__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        voskros__srv__SetGrammar_Response__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
voskros__srv__SetGrammar_Response__Sequence__fini(voskros__srv__SetGrammar_Response__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      voskros__srv__SetGrammar_Response__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

voskros__srv__SetGrammar_Response__Sequence *
voskros__srv__SetGrammar_Response__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  voskros__srv__SetGrammar_Response__Sequence * array = (voskros__srv__SetGrammar_Response__Sequence *)allocator.allocate(sizeof(voskros__srv__SetGrammar_Response__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = voskros__srv__SetGrammar_Response__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
voskros__srv__SetGrammar_Response__Sequence__destroy(voskros__srv__SetGrammar_Response__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    voskros__srv__SetGrammar_Response__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
voskros__srv__SetGrammar_Response__Sequence__are_equal(const voskros__srv__SetGrammar_Response__Sequence * lhs, const voskros__srv__SetGrammar_Response__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!voskros__srv__SetGrammar_Response__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
voskros__srv__SetGrammar_Response__Sequence__copy(
  const voskros__srv__SetGrammar_Response__Sequence * input,
  voskros__srv__SetGrammar_Response__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(voskros__srv__SetGrammar_Response);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    voskros__srv__SetGrammar_Response * data =
      (voskros__srv__SetGrammar_Response *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!voskros__srv__SetGrammar_Response__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          voskros__srv__SetGrammar_Response__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!voskros__srv__SetGrammar_Response__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `info`
#include "service_msgs/msg/detail/service_event_info__functions.h"
// Member `request`
// Member `response`
// already included above
// #include "voskros/srv/detail/set_grammar__functions.h"

bool
voskros__srv__SetGrammar_Event__init(voskros__srv__SetGrammar_Event * msg)
{
  if (!msg) {
    return false;
  }
  // info
  if (!service_msgs__msg__ServiceEventInfo__init(&msg->info)) {
    voskros__srv__SetGrammar_Event__fini(msg);
    return false;
  }
  // request
  if (!voskros__srv__SetGrammar_Request__Sequence__init(&msg->request, 0)) {
    voskros__srv__SetGrammar_Event__fini(msg);
    return false;
  }
  // response
  if (!voskros__srv__SetGrammar_Response__Sequence__init(&msg->response, 0)) {
    voskros__srv__SetGrammar_Event__fini(msg);
    return false;
  }
  return true;
}

void
voskros__srv__SetGrammar_Event__fini(voskros__srv__SetGrammar_Event * msg)
{
  if (!msg) {
    return;
  }
  // info
  service_msgs__msg__ServiceEventInfo__fini(&msg->info);
  // request
  voskros__srv__SetGrammar_Request__Sequence__fini(&msg->request);
  // response
  voskros__srv__SetGrammar_Response__Sequence__fini(&msg->response);
}

bool
voskros__srv__SetGrammar_Event__are_equal(const voskros__srv__SetGrammar_Event * lhs, const voskros__srv__SetGrammar_Event * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // info
  if (!service_msgs__msg__ServiceEventInfo__are_equal(
      &(lhs->info), &(rhs->info)))
  {
    return false;
  }
  // request
  if (!voskros__srv__SetGrammar_Request__Sequence__are_equal(
      &(lhs->request), &(rhs->request)))
  {
    return false;
  }
  // response
  if (!voskros__srv__SetGrammar_Response__Sequence__are_equal(
      &(lhs->response), &(rhs->response)))
  {
    return false;
  }
  return true;
}

bool
voskros__srv__SetGrammar_Event__copy(
  const voskros__srv__SetGrammar_Event * input,
  voskros__srv__SetGrammar_Event * output)
{
  if (!input || !output) {
    return false;
  }
  // info
  if (!service_msgs__msg__ServiceEventInfo__copy(
      &(input->info), &(output->info)))
  {
    return false;
  }
  // request
  if (!voskros__srv__SetGrammar_Request__Sequence__copy(
      &(input->request), &(output->request)))
  {
    return false;
  }
  // response
  if (!voskros__srv__SetGrammar_Response__Sequence__copy(
      &(input->response), &(output->response)))
  {
    return false;
  }
  return true;
}

voskros__srv__SetGrammar_Event *
voskros__srv__SetGrammar_Event__create(void)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  voskros__srv__SetGrammar_Event * msg = (voskros__srv__SetGrammar_Event *)allocator.allocate(sizeof(voskros__srv__SetGrammar_Event), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(voskros__srv__SetGrammar_Event));
  bool success = voskros__srv__SetGrammar_Event__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
voskros__srv__SetGrammar_Event__destroy(voskros__srv__SetGrammar_Event * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    voskros__srv__SetGrammar_Event__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
voskros__srv__SetGrammar_Event__Sequence__init(voskros__srv__SetGrammar_Event__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  voskros__srv__SetGrammar_Event * data = NULL;

  if (size) {
    data = (voskros__srv__SetGrammar_Event *)allocator.zero_allocate(size, sizeof(voskros__srv__SetGrammar_Event), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = voskros__srv__SetGrammar_Event__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        voskros__srv__SetGrammar_Event__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
voskros__srv__SetGrammar_Event__Sequence__fini(voskros__srv__SetGrammar_Event__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      voskros__srv__SetGrammar_Event__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

voskros__srv__SetGrammar_Event__Sequence *
voskros__srv__SetGrammar_Event__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  voskros__srv__SetGrammar_Event__Sequence * array = (voskros__srv__SetGrammar_Event__Sequence *)allocator.allocate(sizeof(voskros__srv__SetGrammar_Event__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = voskros__srv__SetGrammar_Event__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
voskros__srv__SetGrammar_Event__Sequence__destroy(voskros__srv__SetGrammar_Event__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    voskros__srv__SetGrammar_Event__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
voskros__srv__SetGrammar_Event__Sequence__are_equal(const voskros__srv__SetGrammar_Event__Sequence * lhs, const voskros__srv__SetGrammar_Event__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!voskros__srv__SetGrammar_Event__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
voskros__srv__SetGrammar_Event__Sequence__copy(
  const voskros__srv__SetGrammar_Event__Sequence * input,
  voskros__srv__SetGrammar_Event__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(voskros__srv__SetGrammar_Event);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    voskros__srv__SetGrammar_Event * data =
      (voskros__srv__SetGrammar_Event *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!voskros__srv__SetGrammar_Event__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          voskros__srv__SetGrammar_Event__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!voskros__srv__SetGrammar_Event__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}
